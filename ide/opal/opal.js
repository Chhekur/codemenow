...
if (arg == null) arg = nil;
  ...
  (($a = [typeof(process) === 'object' ? function(s){process.stdout.write(s)} : function(s){document.getElementById("output").innerHTML= s;}]), $b = $scope.get('STDOUT'), $b['$write_proc='].apply($b, $a), $a[$a.length-1]);
  (($a = [typeof(process) === 'object' ? function(s){process.stderr.write(s)} : function(s){document.getElementById("output").innerHTML= s}]), $b = $scope.get('STDERR'), $b['$write_proc='].apply($b, $a), $a[$a.length-1]);
