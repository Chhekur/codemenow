...
<html> 
<head> 
 <title> Codemenow | <?php echo explode(",", file_get_contents("tasks/sql/inf/"   . htmlspecialchars($_GET["id"]) . ".txt"))[0] ?> </title>
...
function Continue(){
  var one = 1;
    <?php if($_SESSION['sql']<$_GET['id']){
          $temp = $_GET['id'] + "<script>one;<script/>";
          mysqli_query($con,"UPDATE users SET sqlite='$temp' WHERE id='$usr_id'");        
            } ?>
      redirect();
}
function onFailure(){
 ...
 function giveHint(){
           Materialize.toast('<?php echo explode(",", file_get_contents("tasks/sql/inf/"   . htmlspecialchars($_GET["id"]) . ".txt"))[2] ?>', 16000, "toastHint");
         }
function onSuccess(){
...
function runit() { 
  ...
  mypre=prog.vm.OUTPUT_BUFFER;
   ...
   if(mypre.indexOf(<?php echo explode(",", file_get_contents("tasks/sql/inf/"   . htmlspecialchars($_GET["id"]) . ".txt"))[1] ?>)>=0){
    onSuccess();
   }
   else{
    if(mypre == undefined){
...
<pre id="output" style="color:white; width:40%;float:right;height: 50%;border: 2px solid lightgray; background-color: white" ></pre> 

           <textarea id="commands"><?php echo file_get_contents('tasks/sql/code/' . htmlspecialchars($_GET["id"]) . '.txt') ?></textarea>

               <a id="execute" class="waves-effect waves-light btn" style="width: 9%; position: absolute; bottom: 2%; z-index:5" onclick="runit()" href="#">Check</a>
            ...
            <button id="executes" class="button" style="display: none;">Execute</button>
<button id='savedb'  class="button" style="display: none;">Save the db</button>
<label class="button" style="display: none;">Load an SQLite database file: <input type='file' id='dbfile' ></label>

<div id="error" class="error" style="display: none;"></div>

<pre id="output" style="display: none;">Results will be displayed here</pre>

<script>

 document.getElementById("lc_theory_text").innerHTML = "<?php echo file_get_contents("tasks/sql/theory/"   . htmlspecialchars($_GET["id"]) . ".txt");?>"
 document.getElementById("lc_task_steps").innerHTML = "<?php echo file_get_contents("tasks/sql/task/" . htmlspecialchars($_GET["id"]) . ".txt");?>"
      </script>

 <script src="/ide/sql/sql.js"></script>
 <script type="text/javascript" src="/ide/sql/gui.js"></script>
</body> 
</html> 