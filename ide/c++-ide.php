...
<html> 
<head> 
 <title> Codemenow | <?php echo explode(",", file_get_contents("tasks/c++/inf/"   . htmlspecialchars($_GET["id"]) . ".txt"))[0] ?> </title>
...
function onFailure(){
 ...
 function giveHint(){
           Materialize.toast('<?php echo explode(",", file_get_contents("tasks/c++/inf/"   . htmlspecialchars($_GET["id"]) . ".txt"))[2] ?>', 16000, "toastHint");
         }
function onSuccess(){
...
function runit() {
 ...
 try{var exitCode = JSCPP.run(code, input, config);
  } catch(err){hasSuccess = false;}
  document.getElementById("output").innerHTML=output;
  console.log(exitCode)
  
   if((output).indexOf(<?php echo explode(",", file_get_contents("tasks/c++/inf/"   . htmlspecialchars($_GET["id"]) . ".txt"))[1] ?>)>=0){
    hasSuccess=true;
   	onSuccess();
   }
   else{
   ...
   if(hasSuccess == false){
    document.getElementById("output").innerHTML= "Syntax Error in Submission";
...
<div class="row">
         <div id="pagecontent">
         <?php if(($_GET['id']<$_SESSION['cplusplus'])){?>
...
<pre id="output" style="color:white; width:40%;float:right;height: 50%;border: 2px solid lightgray; background-color: black" ></pre>         
              <textarea id="editor" style="overflow: hidden;"><?php echo file_get_contents('tasks/c++/code/' . htmlspecialchars($_GET["id"]) . '.txt') ?></textarea><textarea  placeholder="User Input" id ="input" style="float:right; width: 40%;color:white;resize: none; outline:none;overflow: hidden; margin-top: -150px;height: 145px;"></textarea>
...
<script>
 ...
 document.getElementById("lc_theory_text").innerHTML = "<?php echo file_get_contents("tasks/c++/theory/"   . htmlspecialchars($_GET["id"]) . ".txt");?>"
 document.getElementById("lc_task_steps").innerHTML = "<?php echo file_get_contents("tasks/c++/task/" . htmlspecialchars($_GET["id"]) . ".txt");?>"
      </script>
<script>
